...
import {
  ...
  CONNECTION_REQUEST_ACCEPTED_TEMPLATE,
...
export const sendVerficationEmail = async (email, verificationToken) => {
  try {
    send_Mail(
      email,
      "Your TravelBuddy verification code",
      "",
...
export const sendPasswordResetEmail = async (email, resetURL) => {
  try {
    send_Mail(
      ...
      "Reset Password",
      "",
      PASSWORD_RESET_TEMPLATE.replace("{resetURL}", resetURL)
    );
  } catch (error) {
...
export const sendConnectionAcceptedEmail = async (email) => {
  try {
    send_Mail(
      email,
    ...
    throw new Error(
      `Error sending Connection Request Accepted email: ${error}`
    );
  }
};
